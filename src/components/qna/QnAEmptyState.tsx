...
const QnAEmptyState: React.FC<QnAEmptyStateProps> = ({
  ...
  const { t } = useLanguage();
  const hasFilters = searchTerm || selectedStatus !== t('qna_filter_all', '전체');
  ...
  return (
    <div className="bg-white rounded-xl p-8 md:p-12 shadow-sm border border-gray-100 text-center">
      ...
      <h3 className="text-base md:text-lg font-medium text-gray-900 mb-2">
        {hasFilters ? t('no_search_results', '검색 결과가 없습니다') : t('qna_no_questions', '아직 문의사항이 없습니다')}
      </h3>
      <p className="text-gray-500 mb-6 text-sm md:text-base">
        {hasFilters
          ? t('try_different_search', '다른 검색어나 필터를 시도해보세요')
          : t('leave_first_inquiry', '첫 번째 문의를 남겨주세요')
        }
      </p>
      {!hasFilters && user && (
        <button
