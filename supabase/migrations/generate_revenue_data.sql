...
-- Version: 6.2 (Error Hotfix)
-- Description: Creates tables and generates 500 realistic test data records.
--              The data includes a general 10-year upward trend and a more
--              pronounced upward trend specifically within the last 30 days.
...
DECLARE
    ...
    region_rand REAL; -- **FIX**: Moved variable declaration to the main DECLARE block
...
BEGIN
    ...
    FOR i IN 1..record_count LOOP
        ...
        -- This logic ensures a higher concentration of data in the recent past,
        -- especially in the last 30 days, to simulate a clear trend.
        IF i <= record_count * 0.16 THEN -- Guarantee ~16% of records are within the last 30 days.
            random_date := CURRENT_DATE - (floor(random() * 30))::INTEGER * INTERVAL '1 day';
        ELSIF i <= record_count * 0.2 THEN -- Guarantee an additional ~4% are within the last year.
             random_date := CURRENT_DATE - (floor(random() * 335) + 30)::INTEGER * INTERVAL '1 day';
        ELSE -- Distribute the remaining ~80% over the last 10 years.
            random_date := start_date + (random() * (end_date - start_date))::INTEGER;
        END IF;

        -- Weighted category selection (50% chance for '제품 매출')
        IF random() < 0.5 THEN
        ...
        -- Product selection based on the chosen category
        CASE random_category
            WHEN '제품 매출' THEN
                -- Higher chance for a specific product 'RIN-COAT'
                IF random() < 0.4 THEN
        ...
        -- Differentiated revenue scale per category to make data more realistic
        CASE random_category
            WHEN '제품 매출' THEN
        ...
        -- 1. Apply a long-term growth factor for the general 10-year upward trend.
        ...
        -- 2. Apply an additional, stronger growth factor if the date is within the last 30 days.
        IF random_date >= CURRENT_DATE - INTERVAL '30 days' THEN
            recent_growth_factor := 1.0 + ( (30 - (CURRENT_DATE - random_date))::numeric / 30.0 ) * 2.0;
            random_revenue := random_revenue * recent_growth_factor;
        END IF;

        -- **FIX**: Weighted region selection logic corrected
        region_rand := random();
        ...
        -- Insert the generated record into the table
...
-- This function call will generate 500 new test records.
SELECT generate_revenue_test_data(500);

-- 5. Create Indexes for Performance
-- Indexes are crucial for fast query performance on large datasets.
...
-- Use these queries to check if the data has been generated as expected.
...
UNION ALL
SELECT '총 매출 합계', TO_CHAR(SUM(revenue), 'FM999,999,999,999,999') || '원' FROM public.revenue_data;

-- **Verify 30-Day Upward Trend**
-- This query shows the daily total revenue for the last 30 days.
SELECT
    date as "날짜",
    TO_CHAR(SUM(revenue), 'FM999,999,999,999,999') || '원' as "일별 매출 합계",
    COUNT(*) as "거래 건수"
...
SELECT
    ...
    TO_CHAR(SUM(revenue), 'FM999,999,999,999,999') || '원' as "월별 매출 합계"
FROM public.revenue_data
...
SELECT
    ...
    TO_CHAR(SUM(revenue), 'FM999,999,999,999,999') || '원' as "총매출",
    ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM public.revenue_data), 2) || '%' as "비율"
FROM public.revenue_data
GROUP BY category
ORDER BY "건수" DESC;

-- Verify Data Distribution for Quick Selection Periods
